...
class ManagerMobileKey(MobileKey):
    ...
    def manager_search(self):
        ...
        for sql in self.sql_format(baidu, pid, area):
            self.write_data(db, sql)
        for sql in self.sql_format(wetchat, pid, area):
            self.write_data(db, sql)
        for sql in self.sql_format(sougou, pid, area):
            self.write_data(db, sql)

    def sql_format(self, obj, pid, area):
